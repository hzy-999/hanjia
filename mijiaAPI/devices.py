...
def _validate_bool(value, prop_name: str, prop_range=None, value_list=None):
...
def _validate_int(value, prop_name: str, prop_range=None, value_list=None):
    """Validate integer value"""
    value = int(value)
    if prop_range:
        if value < prop_range[0] or value > prop_range[1]:
            raise ValueError(f"{value} 超出数值范围, 应该在 {prop_range[:2]} 之间")
        if len(prop_range) >= 3 and prop_range[2] != 1:
            if (value - prop_range[0]) % prop_range[2] != 0:
                raise ValueError(
                    f"无效的值: {value}, 应该在范围 {prop_range[:2]} 内且步长为 {prop_range[2]}")
    return value


def _validate_float(value, prop_name: str, prop_range=None, value_list=None):
    """Validate float value"""
    value = float(value)
    if prop_range:
        if value < prop_range[0] or value > prop_range[1]:
            raise ValueError(f"{value} 超出数值范围, 应该在 {prop_range[:2]} 之间")
        if len(prop_range) >= 3 and isinstance(prop_range[2], int):
            if int(value - prop_range[0]) % prop_range[2] != 0:
                raise ValueError(
                    f"无效的值: {value}, 应该在范围 {prop_range[:2]} 内且步长为 {prop_range[2]}")
    return value


def _validate_string(value, prop_name: str, prop_range=None, value_list=None):
...
class DevProp:
    ...
    @classmethod
    def from_dict(cls, prop_dict: dict) -> 'DevProp':
        """Create DevProp from dictionary (for backward compatibility)"""
        return cls(
            name=prop_dict["name"],
            desc=prop_dict["description"],
            type=prop_dict["type"] if prop_dict["type"] in ["bool", "int", "uint", "float", "string"] else prop_dict["type"],
            rw=prop_dict["rw"],
            unit=prop_dict["unit"],
            range=prop_dict["range"],
            value_list=prop_dict.get("value-list", None),
            method=prop_dict["method"],
        )
...
class DevAction:
    ...
    @classmethod
    def from_dict(cls, act_dict: dict) -> 'DevAction':
        """Create DevAction from dictionary (for backward compatibility)"""
        return cls(
            name=act_dict["name"],
            desc=act_dict["description"],
            method=act_dict["method"],
        )
...
class mijiaDevice():
    def __init__(
        ...
        for prop in dev_info.get("properties", []):
            prop_obj = DevProp.from_dict(prop)
            name = prop["name"]
        ...
        self.action_list = {
            act["name"]: DevAction.from_dict(act)
    ...
    def set(self, name: str, value: Union[bool, int, float, str]):
        ...
        # Use type validator dispatch table
        validator = _TYPE_VALIDATORS.get(prop.type)
        if validator is None:
            raise ValueError(f"不支持的类型: {prop.type}, 可用类型: bool, int, uint, float, string")
        value = validator(value, name, prop.range, prop.value_list)

        # Validate against value_list if present
        if prop.value_list:
